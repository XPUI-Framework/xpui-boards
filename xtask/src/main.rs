//! The gate for `xpui-boards`.
//!
//! Everything CI checks, in one command, and **only what this repository has
//! to check**. There is no C++ here and no product name to
//! forbid — these crates *are* the device names — so this is the common list
//! and nothing else.
//!
//! ```text
...
/// Bare-metal targets these crates are linted for: they are board data, and
/// board data is read on the board.
const BARE_METAL: [(&str, bool); 2] = [
    ("riscv32imc-unknown-none-elf", true),
    ("thumbv6m-none-eabi", false),
];

/// Every crate here, named rather than `--workspace`: the bare-metal runs
/// compile the vendor crates, and adding one is a line here.
const LINT_CRATES: [&str; 8] = [
    "-p",
    "xpui-boards-core",
    "-p",
